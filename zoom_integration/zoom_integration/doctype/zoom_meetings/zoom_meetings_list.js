...
frappe.listview_settings["Zoom Meetings"] = {
  refresh: function (listview) {
    ...
    if (urlParams.get("loggedin") === "true") {
      ...
      let d = new frappe.ui.Dialog({
        ...
        primary_action(values) {
          ...
          baseUrl.pathname = "/";
          (window.location.href = baseUrl.origin + "/app/zoom-meetings"),
            "Zoom Settings";
        },
      });
      d.show();
    ...
    listview.page.add_inner_button("Sign In with Zoom", async function () {
      //SignIn Url
      let client_id = await frappe.db.get_single_value(
        "Zoom Settings",
        "client_id"
      );
      let redirect_uri = await frappe.db.get_single_value(
        "Zoom Settings",
        "redirect_uri"
      );
      ...
      let d = new frappe.ui.Dialog({
        ...
        primary_action(values) {
          ...
          frappe.call({
            ...
            url:
              baseUrl.origin +
              "/api/method/zoom_integration.zoom_integration.doctype.zoom_meetings.zoom_meetings.create_zoom_meeting",
